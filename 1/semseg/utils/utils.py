...
def cal_flops(model, modals, logger):
    # 假设 B=1, C=3, H=512, W=512
    B, C, H, W = 1, 3, 512, 512
    
    # 创建每个模态的输入张量，并存储在列表中
    x = [torch.zeros(B, C, H, W) for _ in range(len(modals))]
    
    # 将模态堆叠到第二维 (模态维度)，生成形状为 (B, M, C, H, W) 的张量
    ...
    # 打印形状以确认
    logger.info(f"x_stacked shape: {x_stacked.shape}")  # 应输出 (B, M, C, H, W)
    
    # 计算 FLOPs
    logger.info(flop_count_table(FlopCountAnalysis(model, x_stacked)))
