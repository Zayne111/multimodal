...
class MCubeS(Dataset):
    ...
    def __init__(self, root: str = 'autodl-tmp/Mcubes/multimodal_dataset', split: str = 'train', transform = None, modals = ['image', 'aolp', 'dolp', 'nir'], case = None) -> None:
        super().__init__()
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        _img = cv2.imread(rgb, -1)[:, :, ::-1].astype(np.float32) / 255
        _target = cv2.imread(lbl_path, -1)
        _aolp_sin = np.load(x1)
        _aolp_cos = np.load(x1_1)
        _dolp = np.load(x2)
        nir = cv2.imread(x3, -1).astype(np.float32) / 255

        aolp = np.stack([_aolp_sin, _aolp_cos, _aolp_sin], axis=2)  # H x W x 3
        dolp = np.stack([_dolp, _dolp, _dolp], axis=2)             # H x W x 3
        nir = np.stack([nir, nir, nir], axis=2)                    # H x W x 3

        _img, aolp, dolp, nir, _target = [
            data[:, self._left_offset:] for data in [_img, aolp, dolp, nir, _target]
        ]

        img_tensor = torch.from_numpy(_img.transpose(2, 0, 1)).float()
        aolp_tensor = torch.from_numpy(aolp.transpose(2, 0, 1)).float()
        dolp_tensor = torch.from_numpy(dolp.transpose(2, 0, 1)).float()
        nir_tensor = torch.from_numpy(nir.transpose(2, 0, 1)).float()

        multi_modal_tensor = torch.stack([img_tensor, aolp_tensor, dolp_tensor, nir_tensor], dim=0)

        label_tensor = torch.from_numpy(_target).long()

        return multi_modal_tensor, label_tensor


    def transform_tr(self, sample: Tuple[Tensor, Tensor]) -> Tuple[Tensor, Tensor]:
        multi_modal_tensor, label = sample

        # 对每个模态单独应用增强操作
        transformed_modalities = []
        for modality in multi_modal_tensor:
            modality = transforms.functional.normalize(
                modality, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)
            )
            transformed_modalities.append(modality)

        # 堆叠回多模态 Tensor
        multi_modal_tensor = torch.stack(transformed_modalities, dim=0)
...
if __name__ == '__main__':
    ...
    trainset = MCubeS(transform=None, split='train')
    trainloader = DataLoader(trainset, batch_size=2, num_workers=0,  drop_last=False, pin_memory=False)

    for i, (multi_modal_tensor, lbl) in enumerate(trainloader):
        print(f"Batch {i+1}:")
        print(f"Multi-modal tensor shape: {multi_modal_tensor.shape}")  # [B, 4, C, H, W]
        print(f"Label shape: {lbl.shape}")  # [B, H, W]
